...
void menu(void);
void pat(void);
struct address // defining variables
{
    int hno;
    char street[40];
    char city[40];
...
void menu()
{
    ...
    printf("\n\t\t0.EXIT");
    printf("\n\n\tEnter your choice: ");
    fflush(stdin); // clears the data in buffering
    choice=getche();
    switch(choice)
        {
            case '1':
...
void pat()
{
    ...
    recsize=sizeof(p);//  avoids repetation of same patient
    while(1)
        {
            clrscr();
            printf("\n\n\t\t\tPATIENT");
